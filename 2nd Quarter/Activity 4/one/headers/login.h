...
username:
	...
	std::cout << "Password: " << passOut << std::string(6 - passOut.size(), '-') << "\e[u";
	char ch;
	ch = _getch();
	...
	std::cout << "\e[4mPassword\e[24m: " << passOut << "\e[s" << std::string(6 - passOut.size(), '-') << "\e[u";
	ch = _getch();
	while (ch != 13) // 13 is enter
	{
		switch (ch)
	...
	else
	{
		...
		std::string fileName = "./login/business/" + username + ".txt";
